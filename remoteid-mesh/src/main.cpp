 ...
 */
 ...
 void print_compact_message(struct uav_data *UAV) {
   ...
   const unsigned long sendInterval = 5000;  // Only send a Drone message every 5 seconds
   ...
   // Build the Drone line: "Drone:" followed by drone MAC, RSSI, and drone GPS link (if valid)
   ...
   // Send the Drone message over Serial1
   if (Serial1.availableForWrite() >= msg_len) {
     Serial1.println(mesh_msg);
     Serial.println("Sent Drone message:");
     Serial.println(mesh_msg);
   } else {
     Serial.println("Mesh TX buffer full, Drone message skipped.");
   }
   
   // After a 1-second pause, send the Pilot-only message (if pilot coordinates are valid)
   delay(1000);
   if (UAV->base_lat_d != 0.0 && UAV->base_long_d != 0.0) {
     ...
     if (Serial1.availableForWrite() >= pilot_len) {
       ...
       Serial.println("Sent Pilot message:");
       Serial.println(pilot_msg);
     } else {
       Serial.println("Mesh TX buffer full, Pilot message skipped.");
     }
   }
 }
 ...
 void callback(void *buffer, wifi_promiscuous_pkt_type_t type) {
   ...
   if (memcmp(nan_dest, &payload[4], 6) == 0) {
     ...
     while (offset < length) {
       ...
       // Process only the alternate branch (French branch removed)
       if (!printed) {
         if ((typ == 0xdd) &&
