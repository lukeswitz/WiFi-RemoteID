...
static void print_compact_message(struct uav_data *UAV)
{
    ...
    const unsigned long sendInterval = 1500;  // Global delay to avoid spamming mesh
    ...
    if (msg_len < MAX_MESH_SIZE && UAV->lat_d != 0.0 && UAV->long_d != 0.0) {
        // Format as a clickable Google Maps link with comma-separated coordinates (no '@' sign)
